...
def get_gemini_response(
    ...
    Args:
        generative_multimodal_model: The multimodal Gemini model.
        model_input: A list of strings representing the inputs to the model.
        stream: Whether to generate the response in a streaming fashion (returning chunks of text at a time) or all at once. Defaults to True.
        generation_config: Configuration parameters for the model.
        safety_settings: Safety settings for the model.

    Returns:
        If `stream=True`, returns the raw streamed response object.
        If `stream=False`, returns the generated text as a single string.
    """
    ...
    if stream:
        # Return the raw streamed response object
        return response
    else:
